...
function searchByFlag_ztree(treeId, searchConditionId, flag,type) {
    ...
    else if (type == "station" || type == "community" || type == "concentrator" || type == "building" || type == "pressure") {
    	highlightNodes = treeObj.getNodesByFilter(monitorParamPwlFilter); 
        allNodes = treeObj.transformToArray(treeObj.getNodes()); // 所有节点
    ...
    if (searchCondition != "") {
        ...
        if (type == "group" || type == "community") {  // 企业
            ...
            treeObj.expandAll(true);
        }else if (type == "station"  || type == "concentrator" || type == "building" || type == "pressure"){
        	treeObj.hideNodes(allNodes)
            treeObj.showNodes(highlightNodes);
            treeObj.expandAll(true);
        }
...
function monitorParamPwlFilter(node){ // 模糊匹配name,otype
    return (node.otype == "station" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "community" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "concentrator" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "building" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "group" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "pressure" && node.name.indexOf(searchParam) > -1) || 
    (node.otype == "secondwater" && node.name.indexOf(searchParam) > -1)  
}
