	...
	var b = {
		init: function(b) {
			...
			function delClick($element){
				...
				input.val('').focus().css('background','#ffffff');
				/**
				 * author: wushengsong
				 * 这个事件对应点击删除按钮，清空所选数据
				 * 是我后来自己加上去的
			...
			function m(a, b, d) {
				...
				for (q.push("<tbody>"), f = 0; e > f; f++) {
					p = 0, j = "",status = b.value[f].status, thisType = b.value[f].type || "", l = b.value[f][d.idField] || "", m = b.value[f][d.keyField] || "";
					...
					if(status != undefined){
						...
						if(m.indexOf(a[0].value)!=-1){
							if(status == 1){
								j = '<tr class="hover" style="background:#dcf5ff;color:#000000;" data-index="' + f + '" data-type="'+ thisType +'" data-id="' + l + '" data-key="' + m + '">' + j + "</tr>", q.push(j)
							}else{
								j = '<tr class="hover" style="background:#fff8b0;color:#000000" data-index="' + f + '" data-type="'+ thisType +'" data-id="' + l + '" data-key="' + m + '">' + j + "</tr>", q.push(j)
							};
	                    }
					}else{
						if(m.indexOf(a[0].value)!=-1){
	                        j = '<tr data-index="' + f + '" data-type="'+ thisType +'" data-id="' + l + '" data-key="' + m + '">' + j + "</tr>", q.push(j)
						}
					};

				}
				return q.push("</tbody></table>"), o.html(q.join("")).show(), n(a, o, d), o.css("max-height") && Number(o.css("max-height").replace("px", "")) < Number(o.find("table:eq(0)").css("height").replace("px", "")) && Number(o.css("min-width").replace("px", "")) < Number(o.css("width").replace("px", "")) ? o.css("padding-right", "0").find("table:eq(0)").css("margin-bottom", "0") : o.css("padding-right", 0).find("table:eq(0)").css("margin-bottom", 0), c(a, o, d), a
			}
			function n(b, c, e) {
			...
			return a("head:eq(0)").append("<style>." + s.listHoverCSS + "{" + s.listHoverStyle + "}</style>"), r.each(function() {
				...
				return g(this) === !1 ? void console.warn() : (s.showBtn || i.css("border-radius", "4px").parents(".input-group:eq(0)").css("width", "100%").find(".input-group-btn>.btn").hide(), i.removeClass("disabled").attr("disabled", !1).attr("autocomplete", "off"), j.css(s.listStyle), s.inputBgColor || (s.inputBgColor = i.css("background-color")), i.on("keydown", function(b) {
					...
					$(this).siblings().children("ul").css({"min-width":width + "px","max-width":width + "px"});					
					$(window).resize(function() {
						...
						$("#" + btnWhiteID).siblings().children("ul").css({"min-width":ulWidth + "px","max-width":ulWidth + "px"})
					});
					var b, c = a(this).val();
					...
					var btnWhiteID = $(this).parent().siblings("input").attr("id");
					var val = navigator.appVersion.split(";");
					var width;
					if (val.length > 1) {
						if(parseInt(val[1].replace(/[ ]/g, "").replace("MSIE","")) == 9) {
					...
					var id = $(this).parent('div').siblings('input').attr("id");
					i.context.alt = '';
					i.context.value = '';
