            ...
            var columns = [
                    ...
                    },
                    {
                        ...
                        render : function(data, type, row, meta) {
                            ...
                            if(row.model == 'NB1.0')
                            {
                                cfgUrlPath = myTable.enableUrl + row.id + "/nb/";
                            }
                            else{
                                cfgUrlPath = myTable.enableUrl + row.id + "/";
                            }
                            var result = '';
                            //修改按钮
                            ...
                            // 配置按钮
                            result += '<button  href="'+cfgUrlPath+'" data-target="#commonWin" data-toggle="modal"  type="button" class="editBtn editBtn-info"><i class="fa fa-cog"></i>配置</button>&nbsp;';
                            //删除按钮
                            console.log(row.communityid)
                            if(row.communityid == "" || row.communityid === undefined){
                                result += '<button type="button" onclick="myTable.deleteItem(\''
                                    + row.id
                                    + '\')" class="deleteButton editBtn "><i class="fa fa-trash"></i>删除</button>';
                                }else{
