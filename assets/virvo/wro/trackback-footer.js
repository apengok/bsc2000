...
function hideMenuContent(targetMenuId) {
    ...
    $("body").unbind("mousedown", onBodyDownClick);
}
                ...
                if (defaults.isShowHMS) {
                    if (this.$dateDisplay.parent().parent().hasClass('drp-calendar-start')) {
                        ...
                        if (this.dateRangePicker.formatDate(newstartdate).substring(0, 11) == this.dateRangePicker.formatDate(newenddate).substring(0, 11)) {
                            ...
                            //     // 如果是离线报表, 结束时间和开始时间都不能大于当天
                            //     var nowDateTime = defaults.nowDate + " 23:59:59";
                            //     if(this.changetime(RU_time) > this.changetime(nowDateTime)
                            //         || this.changetime(_formateDate) > this.changetime(nowDateTime) ) {
                            //         // 还原结束时间为之前
                            //         return false;
                            //     }
                            // }
                            //
                            // bottomLeftStartTime =  _formateDate;
                            // bottomRightStartTime = RU_time;

                            if (this.changetime(_formateDate) >= this.changetime(RU_time)) {
                              layer.msg("开始时间不能大于结束时间");
                              return false;
                            } else {
                              // if (this.datedifference(_formateDate, RU_time) > defaults.dateLimit) {
                              //   layer.msg('最多只能查询'+defaults.dateLimit+'天范围的数据！');
                              //   return false;
                              // }

                              // newstartdate = new Date(_formateDate)
                        ...
                        if (this.dateRangePicker.formatDate(newstartdate).substring(0, 11) == this.dateRangePicker.formatDate(newenddate).substring(0, 11)) {
                            ...
                            // if (this.changetime(RL_time) >= this.changetime(_formateDate)) {
                            //     layer.msg("开始时间不能大于结束时间");
                            //     return false;
                            // } else {
                              // if (this.datedifference(RL_time, _formateDate) > defaults.dateLimit) {
                              //   layer.msg('最多只能查询'+defaults.dateLimit+'天范围的数据！');
                              //   return false;
                              // }
                              // newenddate = new Date(_formateDate)
                                //IE兼容日期 wjk 2018/09/29
                                var date = new Date(_formateDate.replace(/-/g,"/"))
                                newenddate = new Date(date);
